#!/usr/bin/env python3
"""
Script pour créer/mettre à jour l'utilisateur merchant dans Keycloak
"""
import json
import requests
import sys
...
def assign_role_to_user(token, user_id, role):
    ...
    response.raise_for_status()
    print(f"[+] Role '{role['name']}' assigne avec succes")
...
def main():
    try:
        ...
        # Assigner le rôle merchant
        print(f"\n[*] Recherche du role '{USER_ROLE}'...")
        role = get_realm_role(token, USER_ROLE)

        if role:
            print(f"[+] Role '{USER_ROLE}' trouve")
            print(f"[*] Assignation du role a l'utilisateur...")
            try:
                assign_role_to_user(token, user_id, role)
            except requests.exceptions.HTTPError as e:
                if e.response.status_code == 409:
                    print(f"[i] L'utilisateur possede deja le role '{USER_ROLE}'")
                else:
                    raise
        else:
            print(f"[!] Role '{USER_ROLE}' non trouve dans le realm")
